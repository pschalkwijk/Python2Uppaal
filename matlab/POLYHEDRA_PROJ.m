...
for i = 1:(q/2) % Loop on regions 
    ...
    for h = 1:2^(n-1)  % Loop on vertices for polyhedron
...
%                         if mod(c,2) == 0 % if angular coordinate index is even
                            if abs(angle_combs(c,h)-(pi/2)) ~= (pi/2)
                                V(h,c+1) = abs(V(h,c))*tan(angle_combs(c,h)-(pi/2)); %sin(angle_combs((c-1),h))*tan(angle_combs(c,h)-(pi/2)); 
                            else
                                for cc = 1:c                            
                                    V(h,cc) = 0; 
                                end
...
%                          else % if angular coordinate index is odd
%                             if abs(angle_combs(c,h)) ~= (pi/2)
%                                 V(h,c+1) = V(h,c)*tan(angle_combs(c,h));
%                             else
%                                 for cc = 1:n %1:c                            
%                                     V(h,cc) = 0; 
%                                 end
                 ...
                 for c = 2:(n-1)
                         ...
                         %if mod(c,2) == 0 % if angular coordinate index is even
                            if abs(theta_med(c)-(pi/2)) ~= (pi/2)
                                x_med(1,c+1) = x_med(1,c)*tan(theta_med(c)-(pi/2)); %sin(theta_med(c-1))*tan(theta_med(c)-(pi/2));                 
                            else
                                for cc = 1:c                            
                                    x_med(1,cc) = 0;
                                end
...
%                          else % if angular coordinate index is odd
%                             if abs(theta_med(c)) ~= (pi/2)
%                                 x_med(1,c+1) = x_med(1,c)*tan(theta_med(c));
%                             else
%                                 for cc = 1:n %1:c                            
%                                     x_med(1,cc) = 0;
%                                 end
...
% 
% for i = 1:q % Loop on region polyhedra
% 
...
unique_medpoint_regions = size(reg_Seq_Polyhedra,2)
