...
class Template:
    ...
    def layout(self, auto_nails=False, nailAngleThreshold=110.0, nailInterDistanceThreshold=1.0):
        ...
        assert self.initlocation is not None
    ...
    def sharpen(self, angleThreshold, lengthThreshold):
        ...
        while True: # do while? 
            ...
            for (prev, curnail, next) in zip(
                    [(self.source.xpos, self.source.ypos)] + [nail_to_pos(nail) for nail in self.nails[:-1]],
                    self.nails, [nail_to_pos(nail) for nail in self.nails[1:]] + [(self.target.xpos, self.target.ypos)]
                    ):
